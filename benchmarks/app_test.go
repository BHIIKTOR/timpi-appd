...
func setup(db dbm.DB, withGenesis bool, invCheckPeriod uint, opts ...wasmkeeper.Option) (*app.TimpiApp, app.GenesisState) { //nolint:unparam

	logLevel := log.LevelOption(zerolog.InfoLevel)

	wasmApp := app.NewTimpiApp(log.NewLogger(os.Stdout, logLevel), db, nil, true, simtestutil.EmptyAppOptions{}, nil)
...
func SetupWithGenesisAccountsAndValSet(b testing.TB, db dbm.DB, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *app.TimpiApp {
