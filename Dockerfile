# docker build . -t cosmwasm/timpid:latest
# docker run --rm -it cosmwasm/timpid:latest /bin/sh
...
RUN echo "Ensuring binary is statically linked ..." \
  && (file /code/build/timpid | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.17

COPY --from=go-builder /code/build/timpid /usr/bin/timpid
