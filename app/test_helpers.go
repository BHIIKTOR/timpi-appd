...
func setup(t testing.TB, chainID string, withGenesis bool, invCheckPeriod uint, opts ...wasmkeeper.Option) (*TimpiApp, GenesisState) {
	db := dbm.NewMemDB()
	nodeHome := t.TempDir()
	...
	app := NewTimpiApp(log.NewNopLogger(), db, nil, true, appOptions, opts, bam.SetChainID(chainID), bam.SetSnapshot(snapshotStore, snapshottypes.SnapshotOptions{KeepRecent: 2}))
	if withGenesis {
...
func NewWasmAppWithCustomOptions(t *testing.T, isCheckTx bool, options SetupOptions) *TimpiApp {
	t.Helper()
	...
	app := NewTimpiApp(options.Logger, options.DB, nil, true, options.AppOpts, options.WasmOpts)
...
func Setup(t *testing.T, opts ...wasmkeeper.Option) *TimpiApp {
	t.Helper()
...
func SetupWithEmptyStore(t testing.TB) *TimpiApp {
...
func GenesisStateWithSingleValidator(t *testing.T, app *TimpiApp) GenesisState {
	t.Helper()
...
func AddTestAddrsIncremental(app *TimpiApp, ctx sdk.Context, accNum int, accAmt sdkmath.Int) []sdk.AccAddress {
	return addTestAddrs(app, ctx, accNum, accAmt, simtestutil.CreateIncrementalAccounts)
}

func addTestAddrs(app *TimpiApp, ctx sdk.Context, accNum int, accAmt sdkmath.Int, strategy simtestutil.GenerateAccountStrategy) []sdk.AccAddress {
...
func initAccountWithCoins(app *TimpiApp, ctx sdk.Context, addr sdk.AccAddress, coins sdk.Coins) {
...
func NewTestNetworkFixture() network.TestFixture {
	...
	app := NewTimpiApp(log.NewNopLogger(), dbm.NewMemDB(), nil, true, simtestutil.NewAppOptionsWithFlagHome(dir), emptyWasmOptions)
